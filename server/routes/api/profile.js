...
router.get('/', async (req, res) => {
  try {
    const profiles = await Profile.find().populate('user', ['name', 'avatar']); // tacking on users name and avatar from user table
    res.json(profiles);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.get('/user/:user_id', async (req, res) => {
  try {
    ...
    }).populate('user', ['name', 'avatar']); // tacking on users name and avatar from user table
    if (!profile) {
